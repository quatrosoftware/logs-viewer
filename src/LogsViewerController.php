<?php
    ...
    class LogsViewerController extends Controller
    {
        ...
        final public function index(Request $request)
        {
             ...
             */
            $log = ! is_null($file) ? $this->parseLog($file) : [
                'logs'   => [],
                'name'   => null,
                'unique' => []
            ];
            ...
            return app('view')->make('logs-viewer::index', [
                'allLogs' => $getAllLogs,
                'log'     => $log,
        ...
        final private function getFiles(): array
        {
            ...
            foreach ($dir as $file) {
                if (! $file->isDot() && $file->getExtension() == 'log') {
                    ...
                    $files[array_first($name)][] = $file->getFilename();
                    rsort($files[array_first($name)]);
                }
            }
        ...
        final private function parseLog(string $file)
        {
            ...
            $detailedLog = collect($detailedLog)->sortByDesc('date')->keyBy('hash')->toArray();
